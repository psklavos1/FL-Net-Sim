...
RUN_CLIENT_COUNTS=(1 2 4 8 16 32 64 128)
UNISON_MODES=("on" "off")
RUN_UNISON_MODES=("on" "off")
TEMP_CFG_FILES=()

usage() {
  cat <<EOF2
Usage: $(basename "$0") [--dry-run-only] [--keep-records] [--force] [--clients 1,2,4,...] [--unison-modes on,off]
...
Options:
  ...
  --clients           Comma-separated subset of client-count points (default: 1,2,4,8,16,32,64,128).
  --unison-modes      Comma-separated subset of modes (default: on,off).
  ...
  while (($#)); do
    case "$1" in
      ...
      --clients)
        ...
        IFS=',' read -r -a RUN_CLIENT_COUNTS <<<"$1"
        shift
        ;;
      --unison-modes)
        shift
        if (($# == 0)); then
          echo "missing value for --unison-modes" >&2
          exit 2
        fi
        IFS=',' read -r -a RUN_UNISON_MODES <<<"$1"
        shift
        ;;
      -h|--help)
        usage
...
is_allowed_count() {
  local value="$1"
  local allowed
  for allowed in "${CLIENT_COUNTS[@]}"; do
...
is_allowed_unison_mode() {
  local value="$1"
  local allowed
  for allowed in "${UNISON_MODES[@]}"; do
    if [[ "$allowed" == "$value" ]]; then
      return 0
    fi
  done
...
ap_count_for_clients() {
  ...
  case "$c" in
    ...
    128) echo 7 ;;
    *)
      echo "unsupported client count: $c" >&2
      exit 2
      ;;
  esac
}

base_config_id_for_counts() {
  local clients="$1"
  local aps="$2"
  printf 'wifi_c%03d_ap%02d' "$clients" "$aps"
}

config_id_for_mode() {
  local base_id="$1"
  local mode="$2"
  printf '%s_unison_%s' "$base_id" "$mode"
}

validate_client_counts() {
  local normalized=()
  local item trimmed
  for item in "${RUN_CLIENT_COUNTS[@]}"; do
    trimmed="${item//[[:space:]]/}"
    if [[ -z "$trimmed" ]]; then
      continue
    fi
    if ! [[ "$trimmed" =~ ^[0-9]+$ ]]; then
      echo "invalid client count: $trimmed" >&2
      exit 2
    fi
    if ! is_allowed_count "$trimmed"; then
      echo "unsupported client count: $trimmed (allowed: 1,2,4,8,16,32,64,128)" >&2
      exit 2
    fi
  ...
  if ((${#normalized[@]} == 0)); then
    echo "no valid client counts provided" >&2
    exit 2
  fi

  RUN_CLIENT_COUNTS=("${normalized[@]}")
}

validate_unison_modes() {
  local normalized=()
  local item trimmed
  for item in "${RUN_UNISON_MODES[@]}"; do
    trimmed="${item//[[:space:]]/}"
    if [[ -z "$trimmed" ]]; then
      continue
    fi
    if ! is_allowed_unison_mode "$trimmed"; then
      echo "unsupported unison mode: $trimmed (allowed: on,off)" >&2
      exit 2
    fi
  ...
  if ((${#normalized[@]} == 0)); then
    echo "no valid unison modes provided (allowed: on,off)" >&2
    exit 2
  fi

  RUN_UNISON_MODES=("${normalized[@]}")
}
...
main() {
  ...
  local clients mode
  for clients in "${RUN_CLIENT_COUNTS[@]}"; do
    local ap_count base_config_id base_cfg
    ap_count="$(ap_count_for_clients "$clients")"
    base_config_id="$(base_config_id_for_counts "$clients" "$ap_count")"
    base_cfg="$CONFIG_DIR/${base_config_id}.json"

    if [[ ! -f "$base_cfg" ]]; then
      echo "missing config: $base_cfg" >&2
      exit 2
    fi

    for mode in "${RUN_UNISON_MODES[@]}"; do
      local config_id run_cfg_tmp run_cfg
      config_id="$(config_id_for_mode "$base_config_id" "$mode")"
      run_cfg_tmp="$(mktemp)"
      TEMP_CFG_FILES+=("$run_cfg_tmp")
      build_mode_config "$base_cfg" "$mode" "$run_cfg_tmp"
      run_cfg="$run_cfg_tmp"

      local static_key
      static_key="$(compute_static_key "$run_cfg")"
      ...
      echo "[$config_id] clients=$clients aps=$ap_count mode=$mode"
      echo "[$config_id] base_config=$base_cfg"
      echo "[$config_id] run_config=$run_cfg"
      echo "[$config_id] static_key=$static_key"

      start_ns="$(date +%s%N)"
      if ((RUN_DRY_ONLY == 1)); then
        echo "[$config_id] run: python orchestrator.py --config $run_cfg${force_label} --dry-run"
        (cd "$ROOT" && python orchestrator.py --config "$run_cfg" "${force_args[@]}" --dry-run 2>&1 | tee "$capture_log")
      else
        echo "[$config_id] run: python orchestrator.py --config $run_cfg${force_label}"
        (cd "$ROOT" && python orchestrator.py --config "$run_cfg" "${force_args[@]}" 2>&1 | tee "$capture_log")
      fi
      end_ns="$(date +%s%N)"
      ...
      local copied_effective_cfg_json=""
      mkdir -p "$exports_dir"
      cp "$capture_log" "$exports_dir/orchestrator_capture.log"
      copied_effective_cfg_json="$exports_dir/effective_config.json"
      cp "$run_cfg" "$copied_effective_cfg_json"
...
with rows_path.open('r', encoding='utf-8') as f:
    ...
    for item in reader:
        rows.append(
            {
                'config_id': item[0],
                'base_config_id': item[1],
                'client_count': int(item[2]),
                'ap_count': int(item[3]),
                'unison_mode': item[4],
                'config_path': item[5],
                'run_config_path': item[6],
                'static_key': item[7],
                'records_dir': item[8],
                ...
                'copied_effective_config_json': item[25] or None,
                'capture_log': item[26] or None,
            }
        )
...
PY
  ...
  echo ""
  echo "Experiment 4 automation complete."
  echo "Manifest: $manifest_latest"
  while IFS=$'\t' read -r config_id base_config_id clients ap_count mode cfg run_cfg static_key records_dir exec_time rounds_req planned cached deduped par latest_run requested_csv requested_json exports_dir copied_requested_csv copied_requested_json copied_all_rounds_csv copied_all_rounds_json copied_round_index_csv copied_round_states_csv copied_effective_cfg_json capture_log; do
    echo "[$config_id] clients=$clients aps=$ap_count mode=$mode exec_time_s=$exec_time rounds_requested=$rounds_req planned=$planned cached=$cached deduped=$deduped"
